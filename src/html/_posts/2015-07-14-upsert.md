...
Finally, notice that because we're mixing database queries and user logic in a for comprehension, we need to also have an `ExecutionContext` in scope. Our logic will run on the context we supply to the method.
