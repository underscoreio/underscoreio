...
This foray into Scorex was concurrent with my research into blockchain itself, a subject on which I began educating myself - from scratch - not too long ago. So, here are my impressions form the point of view of a mostly-newcomer to the blockchain, and total newcomer to Scorex.
...
The first difficulty a newcomer finds when trying to educate themselves about the blockchain, is mentally separating the various components of any given blockchain system under question, cataloguing them into what they already know and the real novelties. The ideas initially all seem enmeshed together and in a way they are.
...
Also, there is a nice abstraction called a `Box` (more on this later) that represent a cryptographically-protected piece of state, and if one chooses to go along with this abstraction, conveniently there instances of components that employ this abstraction too (but one could just as well develop a different conceptual blueprint from scratch).
